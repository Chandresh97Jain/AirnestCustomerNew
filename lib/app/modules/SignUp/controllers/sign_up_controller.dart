...
class SignUpController extends GetxController {
  ...
  validation(context) {
    if (firstNameTextController.text.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        content: Center(
            child: Textwidget(
          text: "Are you agree to the Terms & Conditions of Airnests",
          color: Colors.white,
        )),
      ));
    } else {
    ...
    try {
      ...
      if (response['status'] == true) {
        ...
        var data = {
          ...
          'address': googleMapService.addressController.value.text,
          'lat': googleMapService.latitude.toString(),
      ...
      if (response['status'] == true) {
        ...
        if (response['first_login'] == "0") {
          await addDocumentWithCustomID(
              response['token'].toString(), firstname, lastname, photoUrl);
        } else {
  ...
  Future<UserCredential> signInWithGoogle(BuildContext context) async {
    ...
    socialSignUpApi("google", googleUser.id, googleUser.email, fname, lname,
        googleUser.photoUrl);
